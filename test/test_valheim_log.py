...
class TestValheimLog(unittest.TestCase):
    ...
    def test_SteamIDFromValheimLogMessage(self):
        steamID = ValheimLogDog.extract_log_parts(self.LogDogObject, self.test_log_message_Steam, self.test_log_date_Steam)
        print(f'steamID: {steamID}')
        self.assertEqual(steamID, '76561197999876368')

    def test_zDOIDFromValheimLogMessage(self):
        zDOID = ValheimLogDog.extract_log_parts(self.LogDogObject, self.test_log_message_zDOID, self.test_log_date_zDOID)
        print(f'ZDOID: {zDOID}')
        self.assertEqual(zDOID, 'Halfdan')

    def test_CharacterDeathFromValheimLogMessage(self):
        zDOID = ValheimLogDog.extract_log_parts(self.LogDogObject, self.test_log_message_char_death, self.test_log_date_zDOID)
        print(f'ZDOID: {zDOID}')
        self.assertEqual(zDOID, 'Halfdan death!')

    def test_ConnectionMessage(self):
        active_connectoins = ValheimLogDog.extract_log_parts(self.LogDogObject, self.test_log_connection_message, self.test_log_date_Steam)
    ...
    def test_DisconnectMessage(self):
        disconnection = ValheimLogDog.extract_log_parts(self.LogDogObject, self.test_log_disconnect_message, self.test_log_date_Steam)
        self.assertEqual(disconnection, '76561197999876368')

    async def test_steamName_from_api(self):
        steamName = await ValheimLogDog.get_steam_persona(self.LogDogObject, self.steamID)
        self.assertEqual(steamName, "76561197999876368")
